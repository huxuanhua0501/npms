...
joForm.handleReadOnly = function(){
	if(joForm.isAdd){
		...
		//新增：当条目锁定，只读 start
		if (joForm.formData["lockStatus"] == 1) {
            joForm.form.find("select").each(function(element,index){
                $(this).attr("disabled",true);
            });
...
joForm.getFormData = function(){
    ...
    console.log("jo-page-form.js-surl:" + sUrl);
    console.log("jo-page-form.js-sdata:" + JSON.stringify(sData));
	var result = jo.postAjax(sUrl,sData);//JSON.stringify(sData)
	console.log("jo-page-form.js-result:" + JSON.stringify(result));
...
joForm.initFormValue = function(oForm,oJson){
	...
	for(var i=0;i<inp.length;i++){
		//获取name
		var element = $(inp[i]);
		console.log("jo-page-form-ele:" + element);
		var key = jo.getDefVal(element.attr("name"),element.attr("id"));
		/*if(jo.isValid(oJson[key])){//值有效才赋值
...
joForm.save = function(sUrl,callBack){
	...
	if(jo.isValid(sUrl)){
		if(!joForm.isAdd){
            ...
            // console.log("保存时提交的数据：" + JSON.stringify(oData));
			//保存前对表单数据的处理
...
joForm.update = function(sUrl){
	...
	if(jo.isValid(sUrl)){
		if(joForm.isAdd){
			...
			//新增：修改前再查询一下，确保是否锁定start
			var getUrl = joForm.params["formDataUrl"];
            var PKName = (joForm.params["PKName"]);
            var PKValue = (joForm.params["PKValue"]);
            var sData = {};
            sData[PKName] = PKValue;
            jo.postAjax(getUrl,sData, function(result){
                if(result.code == 0){
                    joForm.formData["lockStatus"] = result.data[0].lockStatus;
                }else{
                    jo.showMsg(result.info)
                }
            });
			//新增：修改前再查询一下，确保是否锁定end
			//新增：移除select的disabled属性 start
			if (joForm.formData["lockStatus"] == 1) {
                window.parent.jo.showMsg("该条目已被锁定无法修改！");
                window.parent.reloadCurrentPage();//重新加载父页面数据
            ...
            //新增：移除select的disabled属性 end
			var oData = jo.form2Json(joForm.params["formObj"]);
			console.log("修改时提交的数据：" + JSON.stringify(oData));
			//修改前对表单数据的处理
			...
			jo.postAjax(sUrl,oData, function(result){
				if(result.code == 0){
					...
					jo.showMsg(result.info);
					//可在这里刷新一下form：建议于2018-07-17 By JaneYork
				}
			});

...
(function () {
	...
	jo.postAjax = function(sUrl,sData,func,isAsync){
		// console.log("jo.js_surl:" + sUrl);
		// console.log("-------------------------");
		// console.log("jo.js_sdata:" + sData);
		    ...
		    complete : function(jqXHR,textStatus){
		    	if(jo.isValid(jqXHR.responseText)){//后台传的json字符串
    		    	...
    		    	if(jo.isValid(re) && re.code == '-101'){//session过期异常代码
						...
						//top.window.location.href ="page/jsp/old_login.jsp";  //跳转页面
	...
	jo.getUrlParam = function(key){
		...
		if (r != null) {
			if(jo.bIE){
                // console.log(" 获取地址栏参数r[2]" + r[2]);
				return r[2];
			}else{
                // console.log(" 获取地址栏参数decodeURI" + decodeURI(escape(unescape(r[2]))));
...
function Grid(jqGridElement){
	...
	if(_cols && _cols.length > 0){//存在col配置
		...
		// console.log("jo.js-cols-this.cols:" + JSON.stringify(this.cols));//cols列表头
	}
	...
	this.initTitle = function(){
		...
		if(jo.isValid(this.cols)){
			var _headHtml = "<tr>";
			if(jo.isValid(this.titleCss)){
				_headHtml = '<tr class="'+this.titleCss+'">';
			}
			//遍历生产表头
			for(var i=0;i<this.cols.length;i++){
				var col = this.cols[i];
				_headHtml += '<th style="width:' + col.width + ';text-align:'+col.headAlign+';">' + col.title;
				if(col.order){//当前列开启排序功能
					_headHtml += '<span style="display: inline-block;width: 1px;height: 15px;position: relative;">';/*caret*/
					_headHtml += '<div class="order-asc cursor-hand fa fa-angle-up" field="'+col.order+'" grid="'+this.id+'" onclick="GridOrder(this, true)" style="font-weight: bold;height:7px;width: 15px;padding: 0px;margin:0px;position: absolute;top:0px;"><!--<i class="fa fa-caret-up" aria-hidden="true"></i>--></div>';
					_headHtml += '<div class="order-desc cursor-hand fa fa-angle-down" field="'+col.order+'" grid="'+this.id+'" onclick="GridOrder(this, false)" style="font-weight: bold;height:7px;width: 15px;padding: 0px;margin:0px;position: absolute;bottom:0px;"><!--<i class="fa fa-caret-down" aria-hidden="true"></i>--></div>';
					_headHtml += '</span>';
				}
				_headHtml += '</th>';
			}
			_headHtml += "</tr>";
			this.obj.find(">thead").html(_headHtml);
	...
	this.loadJsonArray = function(aJson){
	    console.log("jo.js-loadjson:" + JSON.stringify(aJson));
		this.clearData();//清空数据
		if(jo.isValid(this.cols)){
			for(var m=0;m<aJson.length;m++){
				...
				for(var i=0;i<this.cols.length;i++){
					...
					_html += '<td style="text-align:'+col.align+';">' + _val + '</td>';
				}
				_html += "</tr>";
			...
			if(!jo.isValid(this.data)){
				this.obj.find(">tbody").append("<tr><td colspan='"+this.cols.length+"' style='color:#808080;'>暂无数据！</td></tr>");
			}
		}
	};
	//加载数据
	this.loadData = function(dataOrUrl){
		console.log("jo.js-dataorurl:" + dataOrUrl);
		if(!this.titleInited){
		...
		if(typeof (dataOrUrl) == "object" && dataOrUrl instanceof Array){//传入的数组
			...
			console.log("jo.js-surl:通过URL" +surl);
			if(jo.isValid(surl)){
				var _this = this;//将this引用指向变量,因为这个this的作用域不在ajax回调当中
				jo.postAjax(surl, {}, function(json){
					if(json && json.code == "0"){
						console.log("jo.js-json.data:" + JSON.stringify(json.data));

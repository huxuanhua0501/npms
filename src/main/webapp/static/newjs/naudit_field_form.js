...
joForm.initFormPage = function(oParams){
    ...
    var sID = jo.getDefVal(jo.getUrlParam(sPKName), loginUser.id);
    if(jo.isValid(sID)){
        joForm.isAdd = false;
...
joForm.getFormData = function(){
    ...
    var result = jo.postAjax(sUrl,sData);//正式库信息，对外显示，也用于比较
    var auditData = jo.postAjax("pms/auditRecordBaseinfo/getUserAuditInfo",{"userId":PKValue});//从审核库中获取用户信息，来显示审核的状态
...
joForm.initFormValue = function(oForm,oJson){
    ...
    for(var i=0;i<inp.length;i++){
        ...
        // if(typeof (oJson[key]) != "undefined"){
            ...
            //新改：赋值为审核库的值 end
        //新改：重新赋值一遍，新用户提交时不包含ID和userid start

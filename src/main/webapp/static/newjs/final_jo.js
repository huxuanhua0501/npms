...
(function () {
    ...
    //获取所有
    jo.getAllCheckBoxValue = function (boxName){
        if(!boxName){
            boxName = "ID";
        }
        var ids = $("input[name='"+boxName+"']:checkbox");
        var idStr = "";
        //获取勾选集
        for(var i=0;i<ids.length;i++){
            idStr += "," + ids[i].value;
    ...
    jo.checkForm = function(oForm){
        ...
        //textarea
        emptys = oForm.find("textarea[ErrEmpty]");
        for(var i=0;i<emptys.length;i++){
            var e = emptys[i];
            var val = $(e).val();
            if(val == ""){
        ...
        var ErrYear = oForm.find("input[ErrYear]");
        var reg_erryear = /^(\d{4})$/;
        for(var i=0;i<ErrYear.length;i++){
            var e = ErrYear[i];
            var val = $(e).val();//值

            // if(val != ""){//可以不填
                if(!reg_erryear.test(val)){
                    jo.showMsg(jo.getDefVal($(e).attr("ErrYear"),"请输入正确的年份！"));//提示信息
                    $(e).focus();//获取焦点
                    return false;
                }
            // }
    ...
    this.initTitle = function(){
        ...
        if(jo.isValid(this.cols)){
            ...
            if(jo.isValid(this.titleCss)){
                _headHtml = '<tr class="'+this.titleCss+'">';//TODO 标题行样式
            }
            //遍历生产表头
            for(var i=0;i<this.cols.length;i++){
                var col = this.cols[i];
    ...
    this.loadJsonArray = function(aJson){
        ...
        if(jo.isValid(this.cols)){
            for(var m=0;m<aJson.length;m++){
                ...
                for(var i=0;i<this.cols.length;i++){
                    var col = this.cols[i];
                    var _val = col.field;//col对象配置的field
                    //对可执行字段进行处理
                    if(jo.isValid(_val)){
                        //以 [= 开头, ] 结尾的格式,表示调用函数对字段进行处理,其中的变量(即该行json的字段值)要使用花括号括起来,例如 [=jo.formatDate('{time}')],time为从数据库查询出来的字段
                        if(_val && /^\[=(.)*\]$/.test(_val)){
                            ...
                            _val = jo.getCode(_val, item);
                            console.log("解析{}数据执行getcode方法：" + _val);

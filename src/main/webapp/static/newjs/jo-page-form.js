...
joForm.getFormData = function(){
	...
	if(result != null && result.data.length > 0 && result.data[0]){
        ...
        joForm.formData = result.data;//存放到form.formData,方便调用
	}else{
...
joForm.initFormValue = function(oForm,oJson){
	...
	for(var i=0;i<inp.length;i++){
		...
		if(typeof (oJson[0][key]) != "undefined"){

            element.val(oJson[0][key]);//赋值element.val(oJson[0][key]);//赋值

            if(oJson[0][key] != oJson[1][key]) {
            	element.before("<input type=\"checkbox\" name=\"cb"+ key +"\">");
            }
		}
	}
	//处理radio---------begin

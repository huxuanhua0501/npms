...
/* 此文件为新工具
TODO 主要用于普通表单的赋值操作，和final_rh_audit_field_form.js不同的是赋值方式
简单公共统一操作，如果有扩展，额外加
审核类的用此文件*/
joForm.params = {
...
joForm.getFormData = function(){
    ...
    // var auditData = jo.postAjax(aUrl,{"userId":PKValue});//获取所有，不加限制条件startus=5
...
joForm.initFormValue = function(oForm,oJson){
    ...
    //对复选框赋值 end
    if(typeof(joForm.spliceStatus) == "function"){joForm.spliceStatus(oJson);}
    //在此处理拼接状态显示：start
    ...
    jo.postAjax(sUrl,{"userId":userId},function (json) {
        if (json.code == 0) {
            ...
            $("#selectContent li a").each(function (i,ele) {
                $(ele).children().eq(1).remove();//修复右侧重复叠加
...
joForm.auditChange = function(status) {
    ...
    var oData = jo.form2Json(joForm.params["formObj"]);
    oData.status = status;
    jo.postAjax("pms/auditPmsRelations/update.action",oData,function (json) {
        if (json.code == 0) {
            try{
...
joForm.update = function(sUrl){
    ...
    if(jo.isValid(sUrl)){
        if(joForm.isAdd){
            ...
            //条目锁定提示start
            if (("72b51c53a39744fe9d8b380910c77e63" == loginUser.roleId) && joForm.formData["lockStatus"] == 1) {
            ...
            jo.postAjax(sUrl,oData, function(result){
                if(result.code == 0){
                    if(typeof(joForm.updateSuccessAfter) == "function"){
                        ...
                        window.parent.jo.close("WIN_EDIT");//关闭窗口

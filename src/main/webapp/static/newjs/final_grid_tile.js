...
joView.select = function(){
    ...
    if(jo.isValid(joView.params["dataUrl"])){
        ...
        console.log("平铺类查询区域表单数据打印：" + JSON.stringify(joView.params["formData"]));
        //为了避免查询表单中参数名与排序参数重复
...
joView.loadData = function(url,formData,goPage,pageSize){
    ...
    pageSize = "";    //TODO 将每页显示个数限制去除，暂且没有分页
...
joView.showData = function(oList){
    if(jo.isValid(joView.params["colsInfo"])){
         ...
         * 生成数据行
         */
        if(typeof(joView.setGridData) == "function"){//生成Grid数据接管函数,TODO 不要让它进else
            try{

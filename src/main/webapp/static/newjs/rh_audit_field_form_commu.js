...
joForm.getFormData = function(){
    ...
    var auditData = jo.postAjax("pms/auditRecordCommunication/getUserAuditInfo",{"userId":PKValue,"status":5});
...
joForm.initFormValue = function(oForm,oJson){
        ...
        for (var f=0;f<formAuditData.length;f++) {
            if (key == formAuditData[f]["fieldKey"]){//找到同name
                ...
                if (element.attr("type") != "hidden")  {
                    ...
                    //新改：拼接复选框对checkbox赋值id  status  start
                    // console.log("CHECKBOX" + element.parent().parent().prev().children(":first").val());
                    // element.parent().parent().prev().prepend("<input type=\"checkbox\" id>");
                    var checkboxEle;
                    if (element[0].tagName.toLowerCase() != "textarea") {
                        checkboxEle = element.parent().parent().prev().children(":first");
                    }
                    if (element[0].tagName.toLowerCase() == "textarea") {
                        checkboxEle = element.parent().prev().children(":first");
                    }
                    checkboxEle.attr("value",formAuditData[f]["id"]);
                    checkboxEle.attr("status",formAuditData[f]["auditStatus"]);
                    checkboxEle.attr("lockStatus",formAuditData[f]["lockStatus"]);
                    //新改：对checkbox赋值end
    ...
    if (formAuditData.length > 0) {
        if (typeof (formAuditData[0]["itemId"]) != "undefined") {
            $("#itemId").val(formAuditData[0]["itemId"]);//TODO 虽然设置了，但是目前没有使用
        }
    }
    ...
    for(var i=0;i<inp.length;i++){
        ...
        for (var f=0;f<formAuditData.length;f++) {
            if (key == formAuditData[f]["fieldKey"]){
                element.val(formAuditData[f]["fieldVal"]);
            }
        }
        // }
    }
...
// 新改：处理下拉列表
    ...
    var area = oForm.find("textarea");//处理文本域

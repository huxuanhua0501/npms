    ...
    <script>
        $(function(){
            var jParams = {
                ...
                "addUrl" : "pms/auditPmsHonorTitle/auditInsert.action",//新增
                "deleteUrl" : "pms/auditPmsHonorTitle/delete.action",//删除
                "updateUrl" : "pms/auditPmsHonorTitle/update.action",//修改
                "formDataUrl" : "pms/pmsHonorTitle/get.action",	//正式库信息
                "auditDataUrl" : "pms/auditPmsHonorTitle/get.action",//审核库中的信息,列表类此参数其实没用到
            };

            var sf = jo.getDefVal(jo.getUrlParam("sf"),"");
            console.log("打印表单属性sf："  + sf);
            if (jo.isValid(sf)) {
                jParams["addUrl"] = "pms/auditPmsHonorTitle/auditInsert.action?sf=rs";
                jParams["updateUrl"] = "pms/auditPmsHonorTitle/update.action?sf=rs";
            }
...
<form id="pageForm" class="layui-form layui-row" action="">
    <div class="edit-item layui-col-md12 layui-col-xs12">
        <label class="layui-form-label sex-label">荣誉称号名称</label>
        <div class="layui-input-inline sex-inline lang">
            <input type="text" name="honorName" ErrEmpty placeholder="" autocomplete="off" class="layui-input">
            <input type="hidden" id="userId" name="userId" />
            <input type="hidden" id="id" name="id"/>
        </div>
    </div>
    <div class="edit-item layui-col-md6 layui-col-xs6">
        <label class="layui-form-label">称号级别</label>
        <div class="layui-input-inline">
            <input type="text" name="titleLevel" placeholder="" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div class="edit-item layui-col-md6 layui-col-xs6">
        <label class="layui-form-label">颁发单位</label>
        <div class="layui-input-inline">
            <input type="text" name="awardDept" placeholder="" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div class="edit-item layui-col-md12 layui-col-xs12">
        <label class="layui-form-label">年　　月</label>
        <div class="layui-input-inline lang">
            <input type="text" name="years" placeholder="" ErrBirthBad4 autocomplete="off" class="layui-input" id="time">
        </div>
    </div>

    ...
    <script>
        $(function(){
            var jParams = {
                ...
                "addUrl" : "pms/auditPmsTechnologyBrace/auditInsert.action",//新增
                "deleteUrl" : "pms/auditPmsTechnologyBrace/delete.action",//删除
                "updateUrl" : "pms/auditPmsTechnologyBrace/update.action",//修改
                "formDataUrl" : "pms/technologyBrace/get.action",	//正式库信息
                "auditDataUrl" : "pms/auditPmsTechnologyBrace/get.action"//审核库中的信息,列表类此参数其实没用到
            };

            var sf = jo.getDefVal(jo.getUrlParam("sf"),"");
            if (jo.isValid(sf)) {
                jParams["addUrl"] = "pms/auditPmsTechnologyBrace/auditInsert.action?sf=rs";
                jParams["updateUrl"] = "pms/auditPmsTechnologyBrace/update.action?sf=rs";
            }
...
<form id="pageForm" class="layui-form layui-row" action="">
    <div class="edit-item layui-col-md12 layui-col-xs12">
        ...
        <div class="layui-input-inline lang">
            <input type="text" name="achievementsName" ErrEmpty placeholder="" autocomplete="off" class="layui-input">
            <input type="hidden" id="userId" name="userId" />
            <input type="hidden" id="id" name="id"/>
        </div>
    </div>
    <div class="edit-item layui-col-md6 layui-col-xs6">
        <label class="layui-form-label">发布/采纳部门</label>
        <div class="layui-input-inline">
            <input type="text" name="releaseDept" placeholder="" autocomplete="off" class="layui-input">
        </div>
    </div>

    <div class="edit-item layui-col-md6 layui-col-xs6">
        <label class="layui-form-label">发布/采纳时间</label>
        <div class="layui-input-inline">
            <input type="text" name="release" placeholder="" autocomplete="off" class="layui-input" id="time">
        </div>
    </div>
    <div class="edit-item layui-col-md6 layui-col-xs6">
        <label class="layui-form-label">发布/采纳形式</label>
        <div class="layui-input-inline">
            <select name="releaseName" class="joSelect" data="" dataurl="pms/pmsDictionary/getListByDictionary/RELEASE_NAME" keyfield="dicValue" valuefield="dicValue" firstitem='{"dicValue":"","dicValue":""}'></select>
        </div>
    </div>

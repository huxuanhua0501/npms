...
<head>
    ...
    <script type="text/javascript">
        ...
        joView.setGridData = function (oList) {
            ...
            for (var i=0;i<oList.length;i++) {
                ...
                order_html = '                            <div class="layui-col-md1 paixu">\n' +
                    ...
                    '                            </div>\n' +
                    '                            <i class="iconfont icon-jiahao" style="float:left;padding: 10px;" onclick="addIcon(this)"></i>';
        ...
        function save() {
            ...
            var sUrl = "pms/auditPmsWork/rsBatchUpdate.action";
...
/*            if (jo.isValid(collectStatusIds)) {
                setCollectStatus(collectStatusIds);
            }*/
            if (addCollect) {
        ...
        function dataMerge() {
            ...
            var jsonStr = $("#dataForm").serializeJson();
            var beforeData = $("#content").val();

            var jsonArr = JSON.parse(jsonStr);
            // console.log("接卸数据：" + JSON.stringify(jsonArr));
            var pjItem = "";
            var ids = "";
            for (var i=0;i<jsonArr.length;i++) {
                if (typeof (jsonArr[i]["id"]) != "undefined" && typeof (jsonArr[i]["startTime"]) != "undefined") {
                    pjItem += jsonArr[i]["startTime"] + "--" + jsonArr[i]["stopTime"] + " "+jsonArr[i]["content"] + "\n";
                    var id = jsonArr[i]["id"];
                    $('input[value="'+id+'"]:checkbox').parent().parent().remove();
                    ids += "," + id;
                }
            }
            var afterData = "";
            ...
            if (afterData != "") {
                $("#content").val(beforeData + afterData);
            }else {
                $("#content").val(beforeData);
            }
            window.parent.jo.showMsg("汇总成功！请自行修改或点击保存!");
            //更改状态
            collectStatusIds = ids.substring(1);
        }
        function setCollectStatus(ids,status) {
            console.log("测试ID：" + ids);
            status = 1;
            var sUrl = "pms/auditPmsWork/rsBatchUpdate.action";
            if(jo.isValid(sUrl)){
    ...
    <div class="layui-row">
        <form class="layui-form layui-form-box layui-col-md12 person">
            <div class="person-tit clear">
                <span class="left title">工作经历审核</span>
                <div class="layui-form right">
                    <div class="layui-inline button-group">
                        <button id="collectBtn" type="button" class="layui-btn layui-btn-radius layui-btn-primary" style="background: #378CEF;border-color: #378CEF;color: #fff;" onclick="dataMerge()">汇总信息</button>
                ...
                <div style="width: 100%;margin: 20px auto;text-align: center;">
                    ...
                    <div id="collectDiv" class="">
                        ...
                        <div class="layui-input-inline" style="width: calc(100% - 140px);">
                            ...
                            <textarea class="layui-textarea" name="content" placeholder="格式：起始时间--结束时间 详细内容。例如：2018.09--2019.06 北京大学。说明：时间之间用两个减号分割，内容与时间之间用空格即可，多条记录请按Enter!" id="content" cols="50" style="height:300px;margin-top:10px;"></textarea>
                        </div>
                    </div>
                    <%--汇总end--%>

                    <%--按钮区域start--%>
                    <div id="btnAreaDiv" hidden="hidden" class="person-tit clear" style="margin-top: 10px;margin-bottom: 10px;">
                        <div class="layui-inline">
                            <button type="button" class="layui-btn layui-btn-radius layui-btn-primary add allPass" onclick="joView.auditChangeByUserID(1,'pms/auditPmsWork/batchUpdate.action')">审核通过</button>
                            <button type="button" class="layui-btn layui-btn-radius layui-btn-primary lock allPass" onclick="joView.auditChangeByUserID(2,'pms/auditPmsWork/batchUpdate.action')">审核不通过</button>
                            <button type="button" class="layui-btn layui-btn-radius layui-btn-primary add allPass" onclick="joView.changeStateByUserID(1,'pms/auditPmsWork/lockBatchUpdate.action')">锁定</button>
                            <button type="button" class="layui-btn layui-btn-radius layui-btn-primary lock allPass" onclick="joView.changeStateByUserID(2,'pms/auditPmsWork/lockBatchUpdate.action')">解锁</button>
                        </div>
                    </div>
                    <%--按钮区域end--%>
                    <%--data row--%>
                    <div id="myTileGrid">
                        <table class="layui-table layui-form" id="mainList" dataUrl="pms/auditPmsWork/getPage.action" deleteUrl="pms/auditPmsWork/delete.action" formUrl="page/pms/auditWorkForm.jsp?sf=rs">

...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadInfo(){
            ...
            loadTalentEchelon();
            //加载饼图
            var myChart1 = echarts.init(document.getElementById("talentEchelonPie"));
            myChart1.setOption(option1);

            //加载柱状图
            var myChart2 = echarts.init(document.getElementById("talentEchelonBar"));
        ...
        function loadTalentEchelon(){
            jo.postAjax("pms/statistic/getTalentEchelon",{deptId:deptId,zc:zc,zw:zw,personType:personType}, function(json){
                if(json && json.code == 0){
                    var data = json.data;
                    var info = data[0];
                    //小于30岁
                    ...
                    //加载表格数据
                    var list = [];
                    list.push({option:"<30岁", num:age30});
                    list.push({option:"30-35岁", num:age30_35});
                    list.push({option:"35-40岁", num:age35_40});
                    list.push({option:"40-45岁", num:age40_45});
                    list.push({option:"45-50岁", num:age45_50});
                    list.push({option:"50+岁", num:age50});
                    list.push({option:"尚无填写", num:noInfo});
                    list.push({option:"总数", num:allUserCount});
                    Grid("talentEchelonList").loadData(list);
                }
            });
        }
...
<div class="container-fluid" style="margin: 15px 0px;">
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <div id="talentEchelonPie" style="width: 40%;height: 600px;float: left;"><%--人才梯队饼状图--%>
            </div>

            <div id="talentEchelonBar" style="width: 60%;height: 600px;float: left;"><%--人才梯队柱状图--%>
            </div>

            <div style="float: left;margin: auto;">
                <table class="table table-bordered table-hover table-striped table-condensed" id="talentEchelonList" style="margin: auto;width: 80%;margin-top: 50px;">
                    <col field="_seq" title="排序" width="20%" align=""/>
                    <col field="option" title="年龄区间" width="50%" align=""/>

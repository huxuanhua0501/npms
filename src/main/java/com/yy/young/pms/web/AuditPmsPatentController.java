...
*/
@Controller
@RequestMapping("/pms/auditPmsPatent")
public class AuditPmsPatentController {

    @Resource(name="auditPmsPatentService")
    IAuditPmsPatentService service;
    ...
    ICommonService<Patent> patentService;
    ...
    */
    ...
    @ResponseBody
    public Object getList(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        List<AuditPmsPatent> list = service.getList(obj);
    ...
    @ResponseBody
    public Object oldToAudit(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        List<Patent> educationList = patentService.getList(null);
        for (Patent item : educationList) {
    ...
    */
    ...
    @ResponseBody
    public Object getPage(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        Page page = CommonUtil.getPageFromRequest(request);
        List<AuditPmsPatent> list = service.getPage(obj, page);
        page.setData(list);
    ...
    */
    ...
    public Object get(String id, HttpServletRequest request) throws Exception{
        AuditPmsPatent obj = service.get(id);
    ...
    */
    ...
    @ResponseBody
    public Object count(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
    ...
    */
    ...
    @ResponseBody
    public Object insert(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        service.insert(obj);
    ...
    @ResponseBody
    public Object auditInsert(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            obj.setStatus(1);//新增：不需要审核直接通过，两张表都插入
            //obj.setLockStatus(1);
            service.insert(obj);
            patentService.insert(obj.toAnother());
        }else {
            AuditField AuditField = new AuditField();
            AuditField.setTableName("pms_patent");
    ...
    @ResponseBody
    public Object lockUpdate(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        service.update(obj);
        patentService.update(obj.toAnother());
        return new Result();
    }
    ...
    */
    ...
    @ResponseBody
    public Object update(AuditPmsPatent obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            Patent work = new Patent();
            work.setUserId(obj.getUserId());
            work.setId(obj.getId());
            if (patentService.get(work) != null) {
                obj.setStatus(1);
                //obj.setLockStatus(1);
                patentService.update(obj.toAnother());
            }else {
            ...
            if (auditFieldService.get(AuditField) == null || auditFieldService.get(AuditField).getStatus() != 1) {
                ...
                if (patentService.get(obj.toAnother()) != null) {
                    patentService.update(obj.toAnother());
                }else {
                    patentService.insert(obj.toAnother());
                }
    ...
    public Object importExcel(MultipartFile file, HttpServletRequest request) throws Exception{
        ...
        try {
            if (file != null && !file.isEmpty()) {
                ...
                //从excel读取数据
                List<AuditPmsPatent> list = ei.getImportDataAsBean(AuditPmsPatent.class);
    ...
    */
    ...
    public void exportExcel(AuditPmsPatent obj, HttpServletRequest request, HttpServletResponse response) throws Exception {
        List<AuditPmsPatent> list = service.getList(obj);
        if (list != null && list.size() > 0){
            IExcelExport ee = new ExcelExport();
            ee.insertBeanList(list, AuditPmsPatent.class);
    ...
    public Object lockBatchUpdate(String ids,int lockStatus, HttpServletRequest request) throws Exception{
        ...
        if(StringUtils.isNotBlank(ids)) {
            String[] idArr = ids.split(",");
            List<AuditPmsPatent> list = new ArrayList<AuditPmsPatent>();
            for (int i = 0; i < idArr.length;i++) {
                AuditPmsPatent relations = new AuditPmsPatent();

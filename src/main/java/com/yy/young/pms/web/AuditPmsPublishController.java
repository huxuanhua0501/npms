...
*/
@Controller
@RequestMapping("/pms/auditPmsPublish")
public class AuditPmsPublishController {

    @Resource(name="auditPmsPublishService")
    IAuditPmsPublishService service;
    ...
    ICommonService<Publish> publishService;
    ...
    */
    ...
    @ResponseBody
    public Object getList(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        List<AuditPmsPublish> list = service.getList(obj);
    ...
    @ResponseBody
    public Object oldToAudit(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        List<Publish> educationList = publishService.getList(null);
        for (Publish item : educationList) {
    ...
    */
    ...
    @ResponseBody
    public Object getPage(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        Page page = CommonUtil.getPageFromRequest(request);
        List<AuditPmsPublish> list = service.getPage(obj, page);
        page.setData(list);
    ...
    */
    ...
    public Object get(String id, HttpServletRequest request) throws Exception{
        AuditPmsPublish obj = service.get(id);
    ...
    */
    ...
    @ResponseBody
    public Object count(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
    ...
    */
    ...
    @ResponseBody
    public Object insert(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        service.insert(obj);
    ...
    @ResponseBody
    public Object auditInsert(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            obj.setStatus(1);//新增：不需要审核直接通过，两张表都插入
            //obj.setLockStatus(1);
            service.insert(obj);
            publishService.insert(obj.toAnother());
        }else {
            AuditField AuditField = new AuditField();
            AuditField.setTableName("pms_publish");
    ...
    @ResponseBody
    public Object lockUpdate(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        service.update(obj);
        publishService.update(obj.toAnother());
        return new Result();
    }
    ...
    */
    ...
    @ResponseBody
    public Object update(AuditPmsPublish obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            Publish work = new Publish();
            work.setUserId(obj.getUserId());
            work.setId(obj.getId());
            if (publishService.get(work) != null) {
                obj.setStatus(1);
                //obj.setLockStatus(1);
                publishService.update(obj.toAnother());
            }else {
            ...
            if (auditFieldService.get(AuditField) == null || auditFieldService.get(AuditField).getStatus() != 1) {
                ...
                if (publishService.get(obj.toAnother()) != null) {
                    publishService.update(obj.toAnother());
                }else {
                    publishService.insert(obj.toAnother());
                }
    ...
    public Object importExcel(MultipartFile file, HttpServletRequest request) throws Exception{
        ...
        try {
            if (file != null && !file.isEmpty()) {
                ...
                //从excel读取数据
                List<AuditPmsPublish> list = ei.getImportDataAsBean(AuditPmsPublish.class);
    ...
    */
    ...
    public void exportExcel(AuditPmsPublish obj, HttpServletRequest request, HttpServletResponse response) throws Exception {
        List<AuditPmsPublish> list = service.getList(obj);
        if (list != null && list.size() > 0){
            IExcelExport ee = new ExcelExport();
            ee.insertBeanList(list, AuditPmsPublish.class);
    ...
    public Object lockBatchUpdate(String ids,int lockStatus, HttpServletRequest request) throws Exception{
        ...
        if(StringUtils.isNotBlank(ids)) {
            String[] idArr = ids.split(",");
            List<AuditPmsPublish> list = new ArrayList<AuditPmsPublish>();
            for (int i = 0; i < idArr.length;i++) {
                AuditPmsPublish relations = new AuditPmsPublish();

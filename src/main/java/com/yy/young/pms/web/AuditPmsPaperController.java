...
*/
@Controller
@RequestMapping("/pms/auditPmsPaper")
public class AuditPmsPaperController {

    @Resource(name="auditPmsPaperService")
    IAuditPmsPaperService service;
    ...
    ICommonService<Paper> paperService;
    ...
    */
    ...
    @ResponseBody
    public Object getList(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        List<AuditPmsPaper> list = service.getList(obj);
    ...
    @ResponseBody
    public Object oldToAudit(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        List<Paper> educationList = paperService.getList(null);
        for (Paper item : educationList) {
    ...
    */
    ...
    @ResponseBody
    public Object getPage(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        Page page = CommonUtil.getPageFromRequest(request);
        List<AuditPmsPaper> list = service.getPage(obj, page);
        page.setData(list);
    ...
    */
    ...
    public Object get(String id, HttpServletRequest request) throws Exception{
        AuditPmsPaper obj = service.get(id);
    ...
    */
    ...
    @ResponseBody
    public Object count(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
    ...
    */
    ...
    @ResponseBody
    public Object insert(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        service.insert(obj);
        return new Result();
    }
    ...
    @ResponseBody
    public Object auditInsert(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            obj.setStatus(1);//新增：不需要审核直接通过，两张表都插入
            //obj.setLockStatus(1);
            service.insert(obj);
            paperService.insert(obj.toAnother());
        }else {
            AuditField AuditField = new AuditField();
            AuditField.setTableName("pms_paper");
    ...
    @ResponseBody
    public Object lockUpdate(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        service.update(obj);
        paperService.update(obj.toAnother());
        return new Result();
    }
    ...
    */
    ...
    @ResponseBody
    public Object update(AuditPmsPaper obj, HttpServletRequest request) throws Exception{
        String rs = request.getParameter("sf");
        if (("rs").equals(rs)) {
            Paper work = new Paper();
            work.setUserId(obj.getUserId());
            work.setId(obj.getId());
            if (paperService.get(work) != null) {
                obj.setStatus(1);
                //obj.setLockStatus(1);
                paperService.update(obj.toAnother());
            }else {
            ...
            if (auditFieldService.get(AuditField) == null || auditFieldService.get(AuditField).getStatus() != 1) {
                ...
                if (paperService.get(obj.toAnother()) != null) {
                    paperService.update(obj.toAnother());
                }else {
                    paperService.insert(obj.toAnother());
                }
    ...
    public Object importExcel(MultipartFile file, HttpServletRequest request) throws Exception{
        ...
        try {
            if (file != null && !file.isEmpty()) {
                ...
                //从excel读取数据
                List<AuditPmsPaper> list = ei.getImportDataAsBean(AuditPmsPaper.class);
    ...
    */
    ...
    public void exportExcel(AuditPmsPaper obj, HttpServletRequest request, HttpServletResponse response) throws Exception {
        List<AuditPmsPaper> list = service.getList(obj);
        if (list != null && list.size() > 0){
            IExcelExport ee = new ExcelExport();
            ee.insertBeanList(list, AuditPmsPaper.class);
    ...
    public Object lockBatchUpdate(String ids,int lockStatus, HttpServletRequest request) throws Exception{
        ...
        if(StringUtils.isNotBlank(ids)) {
            String[] idArr = ids.split(",");
            List<AuditPmsPaper> list = new ArrayList<AuditPmsPaper>();
            for (int i = 0; i < idArr.length;i++) {
                AuditPmsPaper relations = new AuditPmsPaper();
